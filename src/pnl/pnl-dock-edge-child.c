...
static void
pnl_dock_edge_child_add (GtkContainer *container,
                         GtkWidget    *widget)
{
  PnlDockEdgeChild *self = (PnlDockEdgeChild *)container;
  ...
  if (!prev_child || !PNL_IS_MULTI_PANED (prev_child))
    {
      ...
      GTK_CONTAINER_CLASS (pnl_dock_edge_child_parent_class)->add (GTK_CONTAINER (self), paned);
...
pnl_dock_edge_child_class_init (PnlDockEdgeChildClass *klass)
{
  ...
  container_class->add = pnl_dock_edge_child_add;
